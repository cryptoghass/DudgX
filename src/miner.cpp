...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("DudgXMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("DudgXMiner : ProcessNewBlock, block not accepted");
...
{
	if(fProofOfStake)
	    LogPrintf("DudgX POS started\n");
	else 
		LogPrintf("DudgXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("DudgX-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("DudgXMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running DudgXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
