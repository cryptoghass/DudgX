...
#if defined(NDEBUG)
#error "DudgX cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight >= 10 && nHeight <= 10080) {
      nSubsidy = 2 * COIN;
    } else if (nHeight >= 10080 && nHeight <= 100000) {
      nSubsidy = 40 * COIN;
...
void static CheckBlockIndex()
{
    ...
    while (pindex != NULL) {
        ...
        assert(nHeight < 10 || (pindex->pskip && (pindex->pskip->nHeight < nHeight)));                                // The pskip pointer must point back for all but the first 2 blocks.
